...
![JVM基本概念](../media/interview/1/JVM基本概念.jpg)
...
![数据区域](../media/interview/1/数据区域.jpg)
...
![垃圾回收算法](../media/interview/1/垃圾回收算法.jpg)
...
![GC垃圾收集器](../media/interview/1/GC垃圾收集器.jpg)
...
![JAVA_IO包](../media/interview/1/JAVA_IO包.jpg)
...
![JAVA_NIO包](../media/interview/1/JAVA_NIO包.jpg)
...
![双亲委派](../media/interview/1/双亲委派.jpg)
...
![集合框架](../media/interview/1/集合框架.jpg)
...
![Java7_HashMap](../media/interview/1/Java7_HashMap.jpg)
...
![Java8实现(引入了红黑树)](../media/interview/1/Java8实现(引入了红黑树).jpg)
...
![Java并发知识库](../media/interview/1/Java并发知识库.jpg)
...
![4种线程池](../media/interview/1/4种线程池.jpg)
...
![线程上下文切换](../media/interview/1/线程上下文切换.jpg)
...
![线程池的组成](../media/interview/1/线程池的组成.jpg)
...
![Java阻塞队列原理](../media/interview/1/Java阻塞队列原理.jpg)

2. 当队列中填满数据的情况下，生产者端的所有线程都会被自动阻塞（挂起），直到队列中有空的位置，线程被自动唤醒。

![Java阻塞队列原理2](../media/interview/1/Java阻塞队列原理2.jpg)

**阻塞队列的主要方法** 

![阻塞队列的主要方法](../media/interview/1/阻塞队列的主要方法.jpg)
...
​    ReentrantLock localReentrantLock = this.lock;     localReentrantLock.lockInterruptibly();     try { 
...
![阻塞队列家族](../media/interview/1/阻塞队列家族.jpg)
...
![禁止指令重排序](../media/interview/1/禁止指令重排序.jpg)
...
适用场景值得说明的是对 volatile 变量的单次读/写操作可以保证原子性的，如 long 和 double 类型变量，但是并不能保证 i++这种操作的原子性，因为本质上 i++是读、写两次操作。在某些场景下可以代替 Synchronized。但是,volatile 的不能完全取代 Synchronized 的位置，只有在一些特殊的场景下，才能适用 volatile。总的来说，必须同时满足下面两个条件才能保证在并发环境的线程安全： 
...
![ThreadLocalMap](../media/interview/1/ThreadLocalMap.jpg)
...
![ConcurrentHashMap](../media/interview/1/ConcurrentHashMap.jpg)
...
![线程调度](../media/interview/1/线程调度.jpg)
...
​    当在作业调度中采用该算法时，每次调度都是从后备作业队列中选择一个或多个最先进入该队列的作业，将它们调入内存，为它们分配资源、创建进程，然后放入就绪队列。在进程调度中采用 FCFS 算法时，则每次调度是从就绪队列中选择一个最先进入该队列的进程，为之分配处理机，使之投入运行。该进程一直运行到完成或发生某事件而阻塞后才放弃处理机，特点是：算法比较简单，可以实现基本上的公平。 2.   短作业(进程)优先调度算法 
...
![高响应比优先调度算法](../media/interview/1/高响应比优先调度算法.jpg)
...
CAS（Compare And Swap/Set）比较并交换，CAS [算法的](http://lib.csdn.net/base/datastructure)过程是这样：它包含 3 个参数 CAS(V,E,N)。V 表示要更新的变量(内存值)，E 表示预期值(旧的)，N 表示新值。当且仅当 V 值等于 E 值时，才会将 V 的值设为 N，如果 V 值和 E 值不同，则说明已经有其他线程做了更新，则当前线程什么都不做。最后，CAS 返回当前 V 的真实值。 
...
![Java异常分类及处理](../media/interview/1/Java异常分类及处理.jpg)
...
```
public static void main(String[] args) {      String s = "abc";      if(s.equals("abc")) {        throw new NumberFormatException();  
    } else {  
...
![反射机制概念](../media/interview/1/反射机制概念.jpg)
...
![JAVA注解](../media/interview/1/JAVA注解.jpg)
...
public class Apple { 
    ...
    private String appleProvider;     public void setAppleProvider(String appleProvider) {         this.appleProvider = appleProvider; 
    } 
...
public class FruitInfoUtil {     public static void getFruitInfo(Class<?> clazz) { 
        ...
        for (Field field : fields) {              if (field.isAnnotationPresent(FruitProvider.class)) { 
...
```
 public class Out {     private static int a;     private int b;     public static class Inner {         public void print() { 
...
```
 public class Out {     private static int a;     private int b;     public class Inner {         public void print() {             System.out.println(a); 
...
![Spring特点](../media/interview/1/Spring特点.jpg)

##  **Spring** 核心组件 

![Spring核心组件](../media/interview/1/Spring核心组件.jpg)

##  **Spring** 常用模块 

![Spring常用模块](../media/interview/1/Spring常用模块.jpg)

## **Spring** 主要包 

![Spring主要包](../media/interview/1/Spring主要包.jpg)

## **Spring** 常用注解 

bean 注入与装配的的方式有很多种，可以通过 xml，get set 方式，构造函数或者注解等。简单易用的方式就是使用 Spring 的注解了，Spring 提供了大量的注解方式。 

![Spring常用注解](../media/interview/1/Spring常用注解.jpg)

##  **Spring** 第三方结合 

![Spring第三方结合](../media/interview/1/Spring第三方结合.jpg)
...
![Spring容器高层视图](../media/interview/1/Spring容器高层视图.jpg)
...
![IOC容器实现](../media/interview/1/IOC容器实现.jpg)
...
![ApplicationContext类继承体系](../media/interview/1/ApplicationContext类继承体系.jpg)
...
![WebApplication体系架构](../media/interview/1/WebApplication体系架构.jpg)
...
![Bean生命周期](../media/interview/1/Bean生命周期.jpg)
...
[彻底征服 Spring AOP 之 理论篇](./子文档/彻底征服 Spring AOP 之 理论篇)

![AOP重要概念](../media/interview/1/AOP重要概念.jpg)
...
![AOP实现原理](../media/interview/1/AOP实现原理.jpg)
...
![MVC流程](../media/interview/1/MVC流程.jpg)
...
![MVC常用注解](../media/interview/1/MVC常用注解.jpg)
...
![两阶段提交](../media/interview/1/两阶段提交.jpg)
...
![Mybatis缓存](../media/interview/1/Mybatis缓存.jpg)
...
![客户端注册](../media/interview/1/zookeeper.jpg)
...
![服务端发现](../media/interview/1/服务端发现.jpg)
...
![API网关](../media/interview/1/API网关.png)
...
![zookeeper配置中心](../media/interview/1/zookeeper配置中心.jpg)

 配置中心数据分类 

![配置中心数据分类](../media/interview/1/配置中心数据分类.jpg)
...
![服务熔断](../media/interview/1/服务熔断.jpg)
...
![多路复用通讯方式](../media/interview/1/多路复用通讯方式.jpg)

客户端通信序列图如下：

![客户端通信序列图](../media/interview/1/客户端通信序列图.jpg)
...
![Reactor单线程模型](../media/interview/1/Reactor单线程模型.jpg)
...
![Reactor多线程模型](../media/interview/1/Reactor多线程模型.jpg)
...
![主从Reactor多线程模型](../media/interview/1/主从Reactor多线程模型.jpg)
...
![无锁设计_线程绑定](../media/interview/1/无锁设计_线程绑定.jpg)
...
![NettyRpc实现](../media/interview/1/NettyRpc实现.jpg)
...
![核心流程](../media/interview/1/核心流程.png)
...
socket 先后将 requestB 和 requestA 发送至 server，而 server 可能将 responseB 先返回，尽管 requestB 请求到达时间更晚。我们需要一种机制保证 responseA 丢给ThreadA，responseB 丢给 ThreadB。 

![通讯过程](../media/interview/1/通讯过程.png)

通讯流程 

***requestID\*** 生成***-AtomicLong\*** 

1. client 线程每次通过 socket 调用一次远程接口前，生成一个唯一的 ID，即 requestID（requestID 必需保证在一个 Socket 连接里面是唯一的），一般常常使用 AtomicLong 从 0 开始累计数字生成唯一 ID；存放回调对象***callback\***到全局***ConcurrentHashMap\*** 
...
3.   当线程调用 channel.writeAndFlush()发送消息后，紧接着执行 callback 的 get()方法试图获取远程返回的结果。在 get()内部，则使用 synchronized 获取回调对象 callback 的锁，再先检测是否已经获取到结果，如果没有，然后调用 callback 的 wait()方法，释放 callback 上的锁，让当前线程处于等待状态。 监听消息的线程收到消息，找到***callback\***上的锁并唤醒 
...
![ProtoclolBuffer特点](../media/interview/1/ProtoclolBuffer特点.jpg)
...
![Thrift](../media/interview/1/Thrift.jpg)

#  网络 

## 网络7层架构 

![img](../media/interview/1/clip_image002.jpg)
...
![TCP_IP原理](../media/interview/1/TCP_IP原理.jpg)
...
![数据包说明](../media/interview/1/数据包说明.jpg)
...
![三次握手](../media/interview/1/三次握手.jpg)
...
![4次挥手](../media/interview/1/4次挥手.jpg)
...
![高层数据](../media/interview/1/高层数据.jpg)
...
![HTTPS](../media/interview/1/HTTPS.jpg)
...
![管理系统](../media/interview/1/管理系统.jpg)
...
![Observer](../media/interview/1/Observer.png)
...
4. Broadcast（广播阶段）：到了这个阶段，Zookeeper 集群才能正式对外提供事务服务，并且 leader 可以进行消息广播。同时如果有新的节点加入，还需要对新节点进行同步。
   ...
   ***ZAB\*** 协议***JAVA\*** 实现（**FLE-****发现阶段和同步合并为 Recovery Phase（恢复阶段**）协议的 Java 版本实现跟上面的定义有些不同，选举阶段使用的是 Fast Leader Election（FLE），它包含了 选举的发现职责。因为 FLE 会选举拥有最新提议历史的节点作为 leader，这样就省去了发现最新提议的步骤。实际的实现将 发现阶段 和 同步合并为 Recovery Phase（恢复阶段）。所以，ZAB 的实现只有三个阶段：Fast Leader Election；Recovery Phase；Broadcast Phase。 
...
4.  一旦 leader 已经和多数的 follower 进行了状态同步后，他就可以开始广播消息了，即进入广播状态。这时候当一个 server 加入 zookeeper 服务中，它会在恢复模式下启动，发现 leader，并和 leader 进行状态同步。待到同步结束，它也参与消息广播。Zookeeper服务一直维持在 Broadcast 状态，直到 leader 崩溃了或者 leader 失去了大部分的 followers 支持。 
...
![Kafka](../media/interview/1/Kafka.png)
...
![Kafka数据存储设计](../media/interview/1/Kafka数据存储设计.jpg)
...
![生产者设计](../media/interview/1/生产者设计.jpg)
...
![消费者设计](../media/interview/1/消费者设计.jpg)
...
![RabbitMQ架构](../media/interview/1/RabbitMQ架构.png)
...
![topic交换器](../media/interview/1/topic交换器.jpg)
...
![Hbase核心架构](../media/interview/1/Hbase核心架构.jpg)
...
![Region寻址方式](../media/interview/1/Region寻址方式.jpg)
...
![Hbase的写逻辑](../media/interview/1/Hbase的写逻辑.jpg)

从上图可以看出氛围 3 步骤： 

获取***RegionServer\*** 第 1 步：Client 获取数据写入的 Region 所在的 RegionServer 请求写***Hlog\*** 第 2 步：请求写 Hlog, Hlog 存储在 HDFS，当 RegionServer 出现异常，需要使用 Hlog 来恢复数据。 
...
3. 前面说到 Hlog 为了保证 Hbase 数据的一致性，那么如果 Hlog 太多的话，会导致故障恢复的时间太长，因此 Hbase 会对 Hlog 的最大个数做限制。当达到 Hlog 的最大个数的时候，会强制刷盘。这个参数是 hase.regionserver.max.logs，默认是 32 个。手工触发 
...
![MongoDB概念](../media/interview/1/MongoDB概念.png)
...
![Cassandra一致Hash和虚拟节点](../media/interview/1/Cassandra一致Hash和虚拟节点.jpg)
...
![数据读请求和后台修复](../media/interview/1/数据读请求和后台修复.jpg)
...
![二级索引](../media/interview/1/二级索引.jpg)
...
[cassandra 3.x官方文档(7)---内部原理之如何读写数据](.\子文档\cassandra 3.x官方文档(7)---内部原理之如何读写数据.md)
...
![数据读取](../media/interview/1/数据读取.png)

行缓存和键缓存请求流程图 

![行缓存和键缓存请求流程图](../media/interview/1/行缓存和键缓存请求流程图.png)
...
![四层负载均衡vs七层负载均衡](../media/interview/1/四层负载均衡vs七层负载均衡.jpg)
...
![ipvs](../media/interview/1/ipvs.jpg)
...
![LVS_NAT模式](../media/interview/1/LVS_NAT模式.jpg)
...
注意：在 NAT 模式中，Real Server 的网关必须指向 LVS，否则报文无法送达客户端**特点：** 
...
![LVS_DR模式](../media/interview/1/LVS_DR模式.jpg)
...
![LVS_TUN模式](../media/interview/1/LVS_TUN模式.jpg)
...
![LVS_FULLNAT模式](../media/interview/1/LVS_FULLNAT模式.jpg)
...
![TokuDB](../media/interview/1/TokuDB.jpg)
...
![1stNF](../media/interview/1/1stNF.jpg)

**第二范式(2nd NF－每个表只描述一件事情)**

首先满足第一范式，并且表中非主键列不存在对主键的部分依赖。 第二范式要求每个表只描述一件事情。 

![2ndNF](../media/interview/1/2ndNF.jpg)

**第三范式(3rd NF－ 不存在对非主键列的传递依赖)** 

 第三范式定义是，满足第二范式，并且表中的列不存在对非主键列的传递依赖。除了主键订单编号外，顾客姓名依赖于非主键顾客编号。

![3rdNF](../media/interview/1/3rdNF.jpg)
...
![柔性事务](../media/interview/1/柔性事务.jpg)

3、 通过将一系列同步的事务操作变为基于消息执行的异步操作, 避免了分布式事务中的同步阻塞操作的影响。 

![异步事务](../media/interview/1/异步事务.jpg)
...
4.   现在服务节点和对象都已经通过同一个 hash 算法映射到 hash 数值空间中了，首先确定对象 hash 值在环上的位置，从此位置沿环顺时针“行走”，第一台遇到的服务器就是其应该定位的服务器。 

![一致性Hsh原理](../media/interview/1/一致性Hsh原理.jpg)
...
![映射关系](../media/interview/1/映射关系.jpg)
...
![插入排序算法1](../media/interview/1/插入排序算法1.jpg)

![插入排序算法2](../media/interview/1/插入排序算法2.jpg)
...
![快速排序算法](../media/interview/1/快速排序算法.jpg)
...
![希尔排序算法](../media/interview/1/希尔排序算法.jpg)
...
![归并排序算法](../media/interview/1/归并排序算法.jpg)
...
![剪枝算法](../media/interview/1/剪枝算法.jpg)
...
![最小生成树算法](../media/interview/1/最小生成树算法.jpg)
...
![插入操作](../media/interview/1/插入操作.jpg)
...
![删除操作](../media/interview/1/删除操作.jpg)
...
![B+Tree](../media/interview/1/B+Tree.jpg)
...
[浅析数据结构-图的基本概念](.\子文档\浅析数据结构-图的基本概念)
...
![RSA](../media/interview/1/RSA.jpg)

![RSA2](../media/interview/1/RSA2.jpg)
...
![MapReduce](../media/interview/1/MapReduce.jpg)
...
![Map_Task执行流程](../media/interview/1/Map_Task执行流程.jpg)
...
![Spark](../media/interview/1/Spark.jpg)
...
![核心组件](../media/interview/1/核心组件.jpg)
...
![SPARK编程模型](../media/interview/1/SPARK编程模型.jpg)
...
![SPARK计算模型](../media/interview/1/SPARK计算模型.jpg)

##  **SPARK** 运行流程 

![SPARK运行流程](../media/interview/1/SPARK运行流程.jpg)
...
![SPARK_RDD流程](../media/interview/1/SPARK_RDD流程.jpg)
...
![Transformation](../media/interview/1/Transformation.jpg)

行动（Action）：Action 算子会触发 Spark 提交作业（Job），并将数据输出 Spark 系统。

![Action](../media/interview/1/Action.jpg)
...
![集群架构](../media/interview/1/集群架构.jpg)
...
![编程模型](../media/interview/1/编程模型.jpg)
...
![Topology运行](../media/interview/1/Topology运行.jpg)
...
![YARN运行流程](../media/interview/1/YARN运行流程.png)
...
![Docker](../media/interview/1/Docker.png)
...
![DOCKER_NETWORK](../media/interview/1/DOCKER_NETWORK.jpg)
...
![DOCKER_NETWORK_TOPOLOGY](../media/interview/1/DOCKER_NETWORK_TOPOLOGY.jpg)
